...
*/
		// Validate cart has cards and has stock.
		if ((!$this->cart->hasCards())  || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
		...
		// Validate balance.
		if($this->customer->getBalance()< $this->cart->getTotal()){
			$redirect = $this->url->link('checkout/cart');
		}
		...
		if (!$redirect) {
			...
			*/
			$order_data['cards'] = array();

			foreach ($this->cart->getCards() as $card) {
				/*$option_data = array();

				foreach ($card['option'] as $option) {
					$option_data[] = array(
						'card_option_id'       => $option['card_option_id'],
						'card_option_value_id' => $option['card_option_value_id'],
				...
				$order_data['cards'][] = array(
					...
					'subtract'   => $card['subtract'],
					'price'      => $card['price'],
					'total'      => $card['total'],
					//'tax'        => $this->tax->getTax($card['price'], $card['tax_class_id']),
					//'reward'     => $card['reward']
				);
			}
...
// 			$data['payment'] = $this->load->controller('payment/' . $this->session->data['payment_method']['code']);
			$data['printcard'] = $this->load->controller('card/print');
